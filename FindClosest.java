    ...
    private PointPair calculateClosestPointPair(Point2D.Double[] p, int startIndex, int lastIndex) {
        ...
        if ((lastIndex - startIndex) < 2) {
            ...
            for (int i = startIndex; i < lastIndex; ++i) {
                PointPair currentMinDistance = new PointPair(p[i], p[i + 1]);
                if (currentMinDistance.getDistance() < result.getDistance()) {
                    result = currentMinDistance;
                }
            }
            return result;
        ...
        PointPair leftHalf = calculateClosestPointPair(p, startIndex, middleIndex);
        PointPair rightHalf = calculateClosestPointPair(p, middleIndex + 1, lastIndex);
        double distance;

        if (leftHalf.getDistance() < rightHalf.getDistance()) {
            distance = leftHalf.getDistance();
            result = leftHalf;
        }
        else {
            distance = rightHalf.getDistance();
            result = rightHalf;
        }
        ...
        int j = 0;
        for (int i = startIndex; i <= lastIndex; i++) {
            if (Math.abs(p[i].getX() - p[middleIndex].getX()) < distance) {
                strip[j] = p[i];
                j++;
            }
        }
